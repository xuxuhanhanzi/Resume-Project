"""Build a single-view, structured frozen evaluation dataset from EvidenceStore."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a strict frozen structured evaluation set")
    ...
    rows = build_strict_eval_rows(records)
    target = Path(args.output)
    count = write_jsonl(target, rows)
    summary = {
        "schema_version": 1,
        "strict_question_records": count,
        "sha256": _sha256(target),
        "image_path_mode": "project_relative" if args.portable_image_root else "source_record",
        "claim_boundary": "one frozen structured target per strict question-level record",
    }
